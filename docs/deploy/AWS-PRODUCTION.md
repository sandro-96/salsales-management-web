...
```
                       ┌────────────────────┐
                       │     Route 53       │  salsales.vn / api.salsales.vn
                       └──────┬─────────────┘
                              │
              ┌───────────────┴───────────────────┐
              ▼                                   ▼
...
- Trong tài liệu này dùng:
  - `salsales.vn` — landing + web app
  - `api.salsales.vn` — backend API
  - `www.salsales.vn` → redirect về root
...
CORS: cho phép GET từ salsales.vn, *.salsales.vn
```
...
[
  {
    ...
    "AllowedOrigins": ["https://salsales.vn", "https://www.salsales.vn"],
    "ExposeHeaders": ["ETag"],
    "MaxAgeSeconds": 3000
  }
]
...
> Caddy tự xin Let's Encrypt cert cho `api.salsales.vn` ngay khi port 80/443 mở và DNS đã trỏ về EIP.
...
app.fe.cors-origins=${FRONTEND_CORS_ORIGINS:https://salsales.vn,https://www.salsales.vn}
...
```env
VITE_API_BASE_URL=https://api.salsales.vn/api
VITE_WS_URL=https://api.salsales.vn/ws
VITE_APP_GOOGLE_CLIENT_ID=<prod client id>
VITE_SITE_URL=https://salsales.vn
...
Alternate domain (CNAME): salsales.vn, www.salsales.vn
...
### 8.3. (Tuỳ chọn) CloudFront cho `api.salsales.vn`
...
```
Route 53 → Hosted zone → Create "salsales.vn"
...
| Tên               | Loại               | Giá trị                                                            |
| ----------------- | ------------------ | ------------------------------------------------------------------ |
| `salsales.vn`     | A — Alias          | CloudFront distribution (web)                                      |
| `www.salsales.vn` | A — Alias          | CloudFront distribution (web)                                      |
| `api.salsales.vn` | A                  | Elastic IP của EC2                                                 |
| `salsales.vn`     | MX (nếu nhận mail) | bỏ qua nếu chỉ gửi outbound                                        |
| `salsales.vn`     | TXT — SPF          | `v=spf1 include:_spf.google.com ~all` (nếu vẫn gửi qua Gmail SMTP) |


> Đợi DNS propagate ~5–30 phút. Verify bằng `dig api.salsales.vn` và `nslookup salsales.vn`.
...
Authorized JavaScript origins:
  https://salsales.vn
  https://www.salsales.vn

Authorized redirect URIs:
  https://salsales.vn/login
  https://www.salsales.vn/login
```
...
- Privacy policy URL: `https://salsales.vn/privacy`
- Terms of Service URL: `https://salsales.vn/terms`
- Authorized domains: `salsales.vn`
...
- `VNPAY_RETURN_URL=https://salsales.vn/billing`
- `MOMO_REDIRECT_URL=https://salsales.vn/billing`
- `MOMO_IPN_URL=https://api.salsales.vn/api/webhook/momo`
...
- Verify domain `salsales.vn` trong SES Console (`ap-southeast-1`) → thêm DKIM CNAME records vào Route 53.
...
```bash
curl -i https://api.salsales.vn/actuator/health     # {"status":"UP"}
curl -i https://salsales.vn                         # 200, HTML SPA
```

Mở browser:

- `https://salsales.vn` → SPA load.
- F12 Network → request đầu tiên tới `https://api.salsales.vn/api/...` 200.
...
- Free: [UptimeRobot](https://uptimerobot.com) ping `https://api.salsales.vn/actuator/health` mỗi 5 phút, gửi email/Telegram khi down.
...
- Caddy auto-issued cert cho `api.salsales.vn`
- S3 `salsales-web-prod` + CloudFront + ACM cert (us-east-1) cho `salsales.vn`, `www.salsales.vn`
...
- VNPay/MoMo production credentials, IPN URL `https://api.salsales.vn/...`
...
- OG image hiển thị khi share Zalo/FB (`https://salsales.vn/og-landing.png` 200)
- Sitemap `https://salsales.vn/sitemap.xml` 200
- Mongo backup cron chạy đêm đầu OK
- CloudWatch alarm + SNS email confirm
- UptimeRobot monitor `api.salsales.vn/actuator/health`
...
Caddy 2 mặc định pass Upgrade — kiểm tra `VITE_WS_URL=https://api.salsales.vn/ws` (https, không wss; SockJS tự upgrade). Nếu vẫn rớt: tăng Caddy `flush_interval -1` trong `reverse_proxy`.
