import { useState } from "react";
 param($m) $inner = ($m.Groups[1].Value + $m.Groups[2].Value) -replace ',\s*,', ',' -replace '^\s*,\s*', '' -replace '\s*,\s*
...
export default BaseWebLayout;
, ''; if ($inner.Trim()) { "import { $($inner.Trim()) } from `"react-router-dom`";`nimport RouteOutletSuspense from `"@/components/routing/RouteOutletSuspense.jsx`";" } else { 'import RouteOutletSuspense from "@/components/routing/RouteOutletSuspense.jsx";' } 
import { useAuth } from "../../hooks/useAuth";
import { useShop } from "../../hooks/useShop";
import {
  FaSignOutAlt,
