...
export default function DropdownComponent({ buttonLabel, items, position = "bottom" }) {
  // chọn class theo position
  const positionClass =
    position === "top"
  ...
  return (
    <Menu as="div" className="relative inline-block text-left w-full">
      ...
      <MenuItems
        transition
        className={`absolute left-0 p-1 z-10 w-56 rounded-lg bg-gray-800 shadow-lg outline-1 outline-offset-1 outline-gray-500 transition 
              ...
              <MenuItem className="rounded-md">
                {({ active }) =>
                  item.to ? (
                    ...
                    >
                      {item.avatar && (
                        <img src={item.avatar} alt="" className="h-5 w-5 rounded-full" />
                      )}
