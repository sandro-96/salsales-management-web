...
const AdminSupportPage = () => {
  ...
  // Auto-open modal khi nhận query ?ticketId=... từ NotificationBell
  useEffect(() => {
  ...
  const columns = [
    {
      ...
      cell: ({ row }) => (
        <div className="max-w-[260px]">
          <div className="font-medium line-clamp-1">{row.original.subject}</div>
          <div className="text-xs text-muted-foreground line-clamp-1">
            {CATEGORY_MAP[row.original.category] || row.original.category}
            {row.original.replyCount > 0 &&
              ` · ${row.original.replyCount} phản hồi`}
          </div>
        </div>
      ),
    },
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Hộp thư hỗ trợ xuyên suốt các shop. Phản hồi và cập nhật trạng thái
          từ đây sẽ được đồng bộ về shop.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {statCards.map((s) => (
          <Card key={s.key} className="p-4">
            ...
            <div className={`text-2xl font-semibold mt-1 ${s.className}`}>
              {stats[s.key] ?? 0}
            </div>
          </Card>
        ))}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="mine">Tôi phụ trách</SelectItem>
          </SelectContent>
        </Select>
      </div>
